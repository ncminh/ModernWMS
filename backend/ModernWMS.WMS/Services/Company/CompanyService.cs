...
namespace ModernWMS.WMS.Services
{
    ...
    public class CompanyService : BaseService<CompanyEntity>, ICompanyService
    {
        ...
        /// <returns></returns>
        public async Task<CompanyViewModel> GetAsync(int id, CurrentUser currentUser)
        {
            var entity = await _dBContext.GetDbSet<CompanyEntity>().AsNoTracking().FirstOrDefaultAsync(t => t.id.Equals(id) && t.tenant_id == currentUser.tenant_id);
            if (entity != null)
            {
        ...
        /// <param name="viewModel">args</param>
        /// <returns></returns>
        public async Task<(bool flag, string msg)> UpdateAsync(CompanyViewModel viewModel, CurrentUser currentUser)
        {
            var DbSet = _dBContext.GetDbSet<CompanyEntity>();
        ...
        {
            var qty = await _dBContext.GetDbSet<CompanyEntity>().Where(t => t.id.Equals(id) && t.tenant_id == currentUser.tenant_id).ExecuteDeleteAsync();
            if (qty > 0)
            {

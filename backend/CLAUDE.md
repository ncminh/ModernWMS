...
- **Module type names are mid-migration from concatenated-lowercase to split PascalCase** (e.g. `Stockadjust` → `StockAdjust`, `Warehousearea` → `WarehouseArea`, `Goodsowner` → `GoodsOwner`), so don't assume every type follows one convention — check the module you're touching. The rename is layered: `IXxxService` interfaces, `XxxController` classes, and concrete `XxxService` classes have been split for every module **except** `Asnmaster` and `Dispatchlist` (the two largest/most complex modules below — still `IAsnmasterService`/`AsnmasterService` and `IDispatchlistService`/`DispatchlistService`/`DispatchlistController`, untouched so far). The `Entities/Models`/`Entities/ViewModels` layer is further behind: only the `Stock*` family has been split there (`StockAdjustEntity`, `StockAdjustViewModel`, …) — every other module, including already-split ones like `GoodsOwner`/`WarehouseArea`, still has old concatenated entity/view-model names (`GoodsownerEntity`, `GoodsownerViewModel`, `WarehouseareaEntity`, …), so a service's own entity/view-model types can be concatenated even when the service class and interface are already split. Routes (`[Route("stockadjust")]`), XML-doc comment text, and DB table names are deliberately left in the old lowercase form; only C# type identifiers get split. Because every entity pins its table via an explicit `[Table("...")]` attribute, renaming an entity class is schema-safe — confirm with `dotnet ef migrations has-pending-model-changes --project ModernWMS --startup-project ModernWMS` after any entity rename, and keep the string-literal type names inside `ModernWMS/Migrations/*.Designer.cs` and `SqlDBContextModelSnapshot.cs` in sync (they aren't auto-updated by a C# rename). Also: on this case-insensitive Windows filesystem, a plain OS-level folder/file case rename does **not** register with git as a rename — use `git mv X X_tmp && git mv X_tmp Y` (two steps, using the git-tracked path's exact casing as the source — `git mv` fails with "not under version control" if the source string doesn't byte-match the index even though the OS resolves it to the same file) or `git status`/`git ls-files` will keep showing the old path as merely modified.
